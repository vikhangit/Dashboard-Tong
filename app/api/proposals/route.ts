...
export async function PATCH(request: Request) {
  try {
    ...
    const updatedProposal = {
      ...
      directionContent:
        directionContent !== undefined
          ? directionContent
          : currentProposal.directionContent,
      updatedAt: new Date(),
    };
    ...
    // Webhook logic
    if (updatedProposal) {
      let action: "APPROVE" | "REJECT" | "DIRECT" | undefined;

      if (status === "approved") action = "APPROVE";
      else if (status === "rejected") action = "REJECT";
      else if (
        directionContent &&
        directionContent !== currentProposal.directionContent
      )
        action = "DIRECT";
      ...
      if (action) {
        await sendWebhook({
          ...
          content:
            action === "DIRECT" ? updatedProposal.directionContent : undefined,
        });
      }
    }
