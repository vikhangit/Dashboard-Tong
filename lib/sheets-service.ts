...
class SheetsService {
  ...
  async readRange(range: string): Promise<any[][]> {
    if (!this.config.spreadsheetId) throw new Error('Spreadsheet ID not configured');
  ...
  private mapStatusToEn(status: string): 'pending' | 'in_progress' | 'completed' {
  ...
  async syncDirectivesToSheet(directives: any[]): Promise<void> {
    ...
    const values = directives.map(d => [
      d.status === 'completed' ? 'Đã hoàn thành' : d.status === 'in_progress' ? 'Đã tiếp nhận' : 'Đã chỉ đạo',
      d.createdAt instanceof Date ? format(d.createdAt, "dd/MM/yyyy HH:mm:ss") : d.createdAt,
      d.category || 'Chung',
      d.content,
      d.assignedTo || '',
      d.deadline instanceof Date ? format(d.deadline, "dd/MM/yyyy HH:mm:ss") : (d.deadline || ''),
      d.actionContent || ''
    ]);
  ...
  async getProposals(): Promise<any[]> {
    ...
    return rows.map((row, index) => {
      ...
      else if (statusRaw === 'Chỉ đạo' || statusRaw === 'Đã chỉ đạo') status = 'directed';
  ...
  async syncProposalsToSheet(proposals: any[]): Promise<void> {
    ...
    const values = proposals.map(p => {
        ...
        return [
            statusLabel,
            (p.createdAt instanceof Date && !isNaN(p.createdAt.getTime())) ? format(p.createdAt, "dd/MM/yyyy") : (p.createdAt || ''),
            (p.updatedAt instanceof Date && !isNaN(p.updatedAt.getTime())) ? format(p.updatedAt, "dd/MM/yyyy HH:mm:ss") : (p.updatedAt || ''),
            p.title,
            p.description,
            p.directionContent || ''
        ];
  ...
  async updateProposal(proposal: any & { id: string }): Promise<void> {
      ...
      const values = [[
          statusLabel,
          (proposal.createdAt instanceof Date && !isNaN(proposal.createdAt.getTime())) ? format(proposal.createdAt, "dd/MM/yyyy") : (proposal.createdAt || ''),
